  ...
  return (
    <div className="container mx-auto p-6">
      ...
      <div className="mb-6 max-w-sm">
        <label htmlFor="defenseTypeFilter" className="block text-sm font-medium text-gray-700 mb-2">
        ...
        <div className="relative">
          <select
            ...
            className="block w-full appearance-none bg-white border border-gray-300 rounded-md py-2 pl-3 pr-10 text-sm font-medium text-gray-700 shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:border-indigo-500 transition-colors hover:border-gray-400"
          >
      ...
      {isLoading ? (
        ...
        <div className="space-y-6">
          {defenseBoards && defenseBoards.length > 0 ? (
                      ...
                      {board.groups.map((group, index) => (
                        <tr key={group._id} className="hover:bg-gray-50 transition-colors">
                          ...
                          <td className="px-4 py-2 text-sm flex flex-col gap-1">
                            {group.members.map(m => <span key={m.studentId}>{m.studentId}</span>)}
                          </td>

                          {/* Student Names */}
                          <td className="px-4 py-2 text-sm flex flex-col gap-1">
                            {group.members.map(m => <span key={m.studentId}>{m.name}</span>)}
                          </td>
            ...
            <p>No {defenseTypeFilter.toLowerCase()} board has been created yet.</p>
          )}
        </div>
      )}
    </div>
